...
```
mkfotaimg.py" -o "${BuildArtifactFileBaseName}.fota" fota.bin "${BuildArtifactFileBaseName}.bin"
```

简化一下

```
mkfotaimg.py" -o app.fota fota.bin app.bin
```

脚本主要功能，读取两个文件app.bin，fota.bin，加上偏移和id，计算在合成的文件中的偏移，再写入文件中。
```
#!/usr/bin/env python
...
if __name__ == "__main__":
    ...
    # 为了简便，命令行里面没有key参数，这一段不考虑
    if args.key:
        with args.key:
            sign_key = ecdsa.keys.SigningKey.from_pem(args.key.read(),
                                                      hashfunc=HASHFUNC)
        args.key = sign_key
    # 打开文件fota.bin, app.bin, app.fota
